...
public class GenerateVRFNameExisting extends Element implements PluginWebSupport {

    String pluginName = "Telkom New WFM - Generate VRF Name Existing - Web Service";
    ...
    public void webService(HttpServletRequest hsr, HttpServletResponse hsr1) throws ServletException, IOException {
        ...
        if ("POST".equals(hsr.getMethod())) {
            try {
                GenerateVRFNameExistingDao dao = new GenerateVRFNameExistingDao();

                if (hsr.getParameterMap().containsKey("wonum")) {
                    String wonum = hsr.getParameter("wonum");
                    String msg = dao.callGenerateVRFNameExisting(wonum);
